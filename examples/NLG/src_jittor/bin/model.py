    ...
    def execute(
        ...
        if lm_labels is not None:
            ...
            if label_smooth > 0.0001:
                ...
                # loss_fct = jt.nn.CrossEntropyLoss(ignore_index=-1)
                loss = jt.nn.cross_entropy_loss(lm_logits.view(-1, lm_logits.size(-1)), lm_labels.view(-1),reduction='none')
                loss = loss.view(_batch, _len)
            if lm_mask is None:
...
if __name__ == "__main__":
    ...
    class LayerNorm(jt.Module):
        def __init__(self, hidden_size, eps=1e-6):
            super().__init__()
            self.gamma = jt.ones(hidden_size)
            self.beta = jt.zeros(hidden_size)
            self.eps = eps
    ...
    # 打印参数
    parameters = layer_norm.parameters()
    ...
    data = [
        ...
        [[10.0, 20.0, 30.0, 40.0],  # 样本2
         [50.0, 60.0, 70.0, 80.0],
         [90.0, 100.0, 110.0, 120.0]]
    ]

    # 2. 构造输入张量（形状：batch_size=2, sequence_length=3, hidden_size=4）
    x = jt.array(data)
