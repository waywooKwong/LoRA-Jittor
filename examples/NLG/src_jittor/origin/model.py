...
class Conv1D(nn.Module):
    def __init__(self, nf, nx):
        ...
        # Debug: 
        # w = jt.empty(nx, nf)
        # init.gauss(shape=(nx,nf), std=0.02)
        # self.weight = w
        self.weight = init.gauss((nx, nf), std=0.02)
...
class GPT2Model(Module):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([nn.deepcopy(block) for _ in range(config.n_layer)])
...
class GPT2LMModel(Module):
    ...
    #         init.gauss(module.weight, mean=0.0, std=0.02)
    #     elif isinstance(module, nn.LayerNorm):
    #         init.zero(module.bias)
    #         init.one(module.weight)
