...
#  ------------------------------------------------------------------------------------------
#  20250701, Jittor version of gpt2_ft.py
#  ------------------------------------------------------------------------------------------
import argparse
import time
import math
...
parser.add_argument(
    "--train_data", required=True, help="location of training data corpus"
)
    ...
    if is_update:
        if args.clip > 0:
            if args.fp16:
                ...
                jt.optim.Optimizer.clip_grad_norm(
                    amp.master_params(_optimizer), args.clip
                )
            else:
                jt.optim.Optimizer.clip_grad_norm(_model.parameters(), args.clip)
...
def train_validate(
    model, optimizer, scheduler, train_loader, valid_loader, args, train_step=0, epoch=0
):
    model.train()
    avg_lm_loss = AverageMeter()
...
if __name__ == "__main__":
    ...
    jt.misc.set_global_seed(args.random_seed)
    ...
    train_data = FT_Dataset(
        args.train_data, args.train_batch_size, args.seq_len, joint_lm=args.obj == "jlm"
    )
    ...
    # # weihua 0701: 1) single GPU, no DistrubutedSampler; 2) turn on shuffle
    ...
    #     #     train_data, seed=args.random_seed
    #     # ),
    # )
    ...
    if args.max_step is None:
        args.max_step = (
            args.max_epoch * train_data.num_batches + args.world_size - 1
        ) // args.world_size
