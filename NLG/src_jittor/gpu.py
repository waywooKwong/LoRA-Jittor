...
""" 
torch 分布式配置: 这个文件中更多实现分布式函数，jittor可以自动单卡转化为MPI的多卡运行，所以这部分代码直接优化掉了
def parse_gpu(args):
...
def distributed_opt(args, model, opt, grad_acc=1):
    if args.platform == "azure":
        ...
        args.platform == "philly" or args.platform == "k8s" or args.platform == "local"
    ):
...
def distributed_gather(args, tensor):
    g_y = [jt.zeros_like(tensor) for _ in range(args.world_size)]
...
def distributed_sync(args):
    if args.platform == "azure":
        args.hvd.allreduce(jt.var(0), name="barrier")
    else:
        args.dist.barrier()
"""
